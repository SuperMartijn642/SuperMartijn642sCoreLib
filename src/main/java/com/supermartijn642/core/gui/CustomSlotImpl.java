...
public class CustomSlotImpl implements CustomSlot {
    ...
    private class VanillaSlot extends Slot {
        ...
        public ItemStack safeInsert(ItemStack stack, int amount){
            if(stack.isEmpty() || !this.mayPlace(stack))
                return stack;
            ItemStack original = CustomSlotImpl.this.getter.get();
            int inserted = CustomSlotImpl.this.inserter.applyAsInt(stack.copyWithCount(amount));
        ...
        public boolean isHighlightable(){
            return CustomSlotImpl.this.showBackground;
        }
    }
    ...
    private static class BuilderImpl implements Builder {
        ...
        @Override
        public Builder itemStorage(int index, Supplier<SlottedStorage<ItemVariant>> storageSupplier){
            this.getter(() -> {
                SlottedStorage<ItemVariant> storage = storageSupplier.get();
                if(index >= storage.getSlotCount())
                    return ItemStack.EMPTY;
                SingleSlotStorage<ItemVariant> slot = storage.getSlot(index);
                long amount = slot.getAmount();
                if(amount < 0)
                    throw new IllegalStateException("Item storage of class '" + storageSupplier.get().getClass().getName() + "' returned negative amount from storage view!");
                return slot.getResource().toStack((int)Math.min(Integer.MAX_VALUE, amount));
            });
            this.inserter(stack -> {
                SlottedStorage<ItemVariant> storage = storageSupplier.get();
                if(index >= storage.getSlotCount())
                ...
                Transaction transaction = Transaction.isOpen() ?
                    ...
                    Transaction.openOuter();
                try(transaction){
                    int inserted = (int)slot.insert(ItemVariant.of(stack), stack.getCount(), transaction);
                    if(inserted < 0)
            ...
            this.extractor(amount -> {
                ...
                try(transaction){
                    ...
                    int extracted = (int)slot.extract(resource, amount, transaction);
                    if(extracted < 0)
                        throw new IllegalStateException("Item storage of class '" + storage.getClass().getName() + "' returned negative amount for #insert for slot at index '" + index + "'!");
                    transaction.commit();
                    return resource.toStack(extracted);

...
public class ScissorWidget extends BaseWidget {
    ...
    private void renderScissored(WidgetRenderContext context, int mouseX, int mouseY, RenderFunction renderFunction){
        if(mouseX < this.x || mouseX > this.x + this.width || mouseY < this.y || mouseY > this.y + this.height)
            mouseX = mouseY = -100;
        int finalMouseX = mouseX;
        int finalMouseY = mouseY;
        ScreenUtils.withScissor(context.poseStack(), this.x, this.y, this.width, this.height, () -> renderFunction.render(context, finalMouseX, finalMouseY));
    }
    ...
    public void renderBackground(WidgetRenderContext context, int mouseX, int mouseY){
        this.renderScissored(context, mouseX, mouseY, super::renderBackground);
    }

    @Override
    public void render(WidgetRenderContext context, int mouseX, int mouseY){
        this.renderScissored(context, mouseX, mouseY, super::render);
    }

    @Override
    public void renderForeground(WidgetRenderContext context, int mouseX, int mouseY){
        this.renderScissored(context, mouseX, mouseY, super::renderForeground);
    }

    @Override
    public void renderOverlay(WidgetRenderContext context, int mouseX, int mouseY){
        this.renderScissored(context, mouseX, mouseY, super::renderOverlay);
    }

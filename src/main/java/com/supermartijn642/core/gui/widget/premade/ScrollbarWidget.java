...
public class ScrollbarWidget extends BaseWidget {
    ...
    @ApiStatus.Internal
    public static final Identifier BACKGROUND = Identifier.fromNamespaceAndPath("supermartijn642corelib", "gui/scrollbar_background");
    @ApiStatus.Internal
    public static final Identifier SCROLLER = Identifier.fromNamespaceAndPath("supermartijn642corelib", "gui/scroller");
    ...
    private final Identifier background, scroller;
    ...
    public void renderBackground(WidgetRenderContext context, GuiGraphicsHelper graphics, int mouseX, int mouseY){
        ...
        if(this.background != null)
            graphics.submitSprite(this.background, this.x - 1, this.y - 1, this.width + 2, this.height + 2);
    }
    ...
    public void render(WidgetRenderContext context, GuiGraphicsHelper graphics, int mouseX, int mouseY){
        ...
        if(range <= 0 || !this.active){
            graphics.submitSprite(this.scroller, this.x, this.y, this.width, this.scrollerHeight, p -> p.uv(0, 2 / 3f, 1, 1 / 3f));
            return;
        }

        // Calculate the target scroller position
        double value = Math.clamp(this.value.getAsDouble() - min, 0, range);
        ...
        if(this.scrollerSpeed != null)
            this.scrollerPosition = (float)Math.clamp(targetPosition, this.scrollerPosition - this.scrollerSpeed, this.scrollerPosition + this.scrollerSpeed);
        else
            this.scrollerPosition = targetPosition;
        float offset = (this.height - this.scrollerHeight) * this.scrollerPosition;
        graphics.submitSprite(this.scroller, this.x, this.y + offset, this.width, this.scrollerHeight, p -> p.uv(0, this.scrollable && this.isFocused() ? 1 / 3f : 0, 1, 1 / 3f));
    }
    ...
    private void tryScrollTo(double targetPosition, boolean fromScrollWheel){
        ...
        if(this.smoothValues && !fromScrollWheel)
            targetPosition = Math.clamp(targetPosition, this.scrollerPosition - this.scrollerSpeed, this.scrollerPosition + this.scrollerSpeed);
        double min = this.minValue.getAsDouble(), max = this.maxValue.getAsDouble();
        double range = max - min;
        if(range <= 0)
            return; // Give up
    ...
    public static class Builder {
        ...
        private Identifier background = BACKGROUND, scroller = SCROLLER;
        ...
        public Builder background(Identifier texture){
        ...
        public Builder scroller(Identifier texture){

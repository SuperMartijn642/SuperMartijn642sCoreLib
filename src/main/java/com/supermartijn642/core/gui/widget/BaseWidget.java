...
public abstract class BaseWidget implements Widget {
    ...
    public void setFocused(boolean focused){
        if(this.focused != focused)
            this.nextNarration = focused ? System.currentTimeMillis() + 750 : Long.MAX_VALUE;
    ...
    public void renderBackground(int mouseX, int mouseY){
        ...
        if(!this.focused)
            ...
            this.focusedWidget = null;
            this.nextNarration = System.currentTimeMillis() + 750;
        }
        ...
        if(this.focused && this.focusedWidget == null && System.currentTimeMillis() > this.nextNarration){
            ITextComponent message = this.getNarrationMessage();
            String s = message == null ? "" : message.getFormattedText();
            if(!s.isEmpty()){
        ...
        this.widgets.stream().filter(w -> w != this.focusedWidget).forEach(w -> w.renderBackground(mouseX, mouseY));
        if(this.focusedWidget != null)
            this.focusedWidget.renderBackground(mouseX, mouseY);
    }

    @Override
    public void render(int mouseX, int mouseY){
        // Render internal widgets
        this.widgets.stream().filter(w -> w != this.focusedWidget).forEach(w -> w.render(mouseX, mouseY));
        if(this.focusedWidget != null)
            this.focusedWidget.render(mouseX, mouseY);
    }

    @Override
    public void renderForeground(int mouseX, int mouseY){
        // Render internal widgets' foreground
        this.widgets.stream().filter(w -> w != this.focusedWidget).forEach(w -> w.renderForeground(mouseX, mouseY));
        if(this.focusedWidget != null)
            this.focusedWidget.renderForeground(mouseX, mouseY);
    }

    @Override
    public void renderOverlay(int mouseX, int mouseY){
        // Render internal widgets
        this.widgets.stream().filter(w -> w != this.focusedWidget).forEach(w -> w.renderOverlay(mouseX, mouseY));
        if(this.focusedWidget != null)
            this.focusedWidget.renderOverlay(mouseX, mouseY);
    }
    ...
    public void renderTooltips(int mouseX, int mouseY){
        if(this.focused){
            if(this.focusedWidget != null)
                this.focusedWidget.renderTooltips(mouseX, mouseY);
            else{
                ...
                ScreenUtils.drawTooltip(tooltips, mouseX, mouseY);
            }
        }
    }

...
public class BlockStatePropertyLootCondition implements LootCondition {
    ...
    public BlockStatePropertyLootCondition(Block block, List<Pair<IProperty<?>,Set<?>>> properties){
        this.block = block;
    ...
    @SafeVarargs
    public BlockStatePropertyLootCondition(Block block, Pair<IProperty<?>,?>... propertyPair){
        this.block = block;
    ...
    public boolean testCondition(Random random, LootContext context){
        IBlockState state = ((LootContextExtension)context).coreLibGetBlockState();
        if(state == null || state.getBlock() != this.block)
            return false;
    ...
    private static class Serializer extends LootCondition.Serializer<BlockStatePropertyLootCondition> {

        private Serializer(){
            super(new ResourceLocation("supermartijn642corelib", "block_state_property"), BlockStatePropertyLootCondition.class);
        }

        @Override
        public void serialize(JsonObject json, BlockStatePropertyLootCondition value, JsonSerializationContext context){
        ...
        @Override
        public BlockStatePropertyLootCondition deserialize(JsonObject json, JsonDeserializationContext context){
            ...
            if(!RegistryUtil.isValidIdentifier(json.get("block").getAsString()))
            ...
            for(Map.Entry<String,JsonElement> entry : json.getAsJsonObject("properties").entrySet()){
                ...
                if(entry.getValue().isJsonPrimitive() && entry.getValue().getAsJsonPrimitive().isString()){
                    com.google.common.base.Optional<?> value = property.parseValue(entry.getValue().getAsString());
                    if(!value.isPresent())
            ...
            return new BlockStatePropertyLootCondition(block, properties);
        }
    }
}
